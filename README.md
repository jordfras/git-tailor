...
Or download a pre-built `gt` binary for Linux (x86_64 or arm64), Windows
(x86_64), or macOS (Apple Silicon or Intel) from the
...
work it finds, saving it under `refs/git-tailor/rescue/*`; `gt --drop-rescued`
lists those with the command that restores each, and removes them. The third
tutorial in the
