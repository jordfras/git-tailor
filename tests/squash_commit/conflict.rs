...
///
/// The shape: the target is old enough that a file added *after* it is absent
/// from its tree. Pausing at a conflict checks out the cherry-pick result,
/// which is rooted in the target — and libgit2's checkout leaves a file that
/// index does not mention sitting on disk. It is untracked from that moment,
/// so when the replay re-adds it the collision guard refuses, naming a file the
/// operation itself left there.
#[test]
